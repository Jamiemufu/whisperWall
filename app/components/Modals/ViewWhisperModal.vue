  ...
  >
    ...
    <template #footer>
      <div class="flex gap-2 pt-2">
        <UButton v-if="whisper" variant="outline" size="xl" color="neutral">Support</UButton>
      </div>
...
const { data: whisper, refresh } = await useAsyncData('whisper', () => $fetch<Whisper & { responses: any[] }>(`/api/whisper/${props.whisperId}`), { immediate: true });
</script>
